...
void CSoundMgr::PlayBGM(const TCHAR* pSoundKey, float fVolume)
{
	...
	{
		wchar_t msg[128];
		swprintf_s(msg, L"사운드 키 [%s] 를 찾을 수 없습니다!", pSoundKey);
		MessageBox(nullptr, msg, L"PlayBGM 실패", MB_OK);
		return;
	}
...
bool CSoundMgr::Is_ChannelPlaying(CHANNELID _id)
{
	FMOD_BOOL bPlay = FALSE;
	if (m_pChannelArr[_id])
		FMOD_Channel_IsPlaying(m_pChannelArr[_id], &bPlay);
...
void CSoundMgr::LoadSoundFile()
{
	...
	while (iResult != -1)
	{
		...
		{
			/*char szMsg[256];
			sprintf_s(szMsg, "성공: %s", szFullPath);
			MessageBoxA(nullptr, szMsg, "FMOD 로딩 성공", MB_OK);*/
